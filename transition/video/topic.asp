...
Dim video_cmd, topicQry

topicQry = "transition"
If Len(Request.QueryString("se")) > 0 Then
	topicQry = Replace(Request.QueryString("se"), "'", "''")
	'topicQry = UCase(Left(topicQry, 1)) &  Mid(topicQry, 2)
End If
...
video_cmd.CommandText = "SELECT transVideos.*, transVideoSeries.title as seriesTitle, transVideoSeries.id as seriesId" & _
	...
	" WHERE (topics LIKE '%" & topicQry & "%' AND transVideos.visible <> 'false') ORDER BY date_posted DESC;"
...
<title>Video Topic: <%=topicQry%> - National Parent Center on Transition and Employment</title>

<meta name="description" content="Video's on '<%=topicQry%>' from the National Parent Center on Transition and Employment">
<!-- for Facebook -->          
<meta property="og:title" content="Video Topic: <%=topicQry%> - National Parent Center on Transition and Employment" />
<meta property="og:type" content="video.other" />
<meta property="og:image" content="http://www.pacer.org/transition/images/npcteLogo-fb.png" />
<meta property="og:url" content="http://www.pacer.org/transition/video/topic.asp?se=<%=topicQry%>"  />
<meta property="og:description" content="Video's on '<%=topicQry%>' from the National Parent Center on Transition and Employment" />
<%End If%>
...
<%If Not videoList.EOF Then%>
    ...
    <div id="topicHeader">
        <h1 id="maincontent">Video Topic: <%=topicQry%></h1>
    </div>
