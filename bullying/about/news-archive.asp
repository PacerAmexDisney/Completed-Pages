...
If  requestedYear = "" Then
	Response.Redirect("/bullying/about/news-archive.asp?year=" & DatePart("yyyy", Now()) - 1)
End If
Set rs_cmd = Server.CreateObject ("ADODB.Command")
rs_cmd.ActiveConnection = MM_PACER_WEB_STRING
rs_cmd.CommandText = "SELECT * FROM dbo.bullyNews WHERE DATEPART(year, posted_date) = " & requestedYear & " ORDER BY posted_date DESC" 
...
<title>National Bullying Prevention Center - Archived News</title>
<link rel="icon" href="http://www.pacer.org/bullying/favicon.ico" />
<link href="/bullying/css/bullying.css" rel="stylesheet" type="text/css" />
<!--#include virtual="/bullying/dynamic-head-items.html"--><link href="/bullying/css/news.css" rel="stylesheet" type="text/css" />
...
<div class="breadcrumb"> <a href="/bullying/">Home</a> / <a href="/bullying/about/news.asp">News</a> /</div>
...
<table width="100%" style="margin-top:15px; font-size:.9em" cellpadding="5">
...
<!-- ###################### END Navigation ############################## -->
  ...
  <tr>
  <td align="center" colspan="4">Page <%=RoundUp(rs_last / 10)%> of <%=RoundUp(rs_total / 10)%>
	</td></tr>
    <tr>
  ...
  <p style="text-align:right; padding-top:5px; padding-right:45px; clear:right">
  <a style="font-size:12px" href="/bullying/about/news.asp">Go To Current News </a>
  </p>
  </div>


    <h1 id="maincontent">Archived News - <%=requestedYear%></h1>            
<form id="selectayear" name="selectayear" action="">
 <label><u>Y</u>ear: </label>
 <select name="list" onchange="goToNewPage(document.selectayear.list)">
 <%
 Dim currYear
 currYear = DatePart("yyyy", Now()) - 1
 Do While currYear > 2009%>
 <option value="/bullying/about/news-archive.asp?year=<%=currYear%>" <%If Cint(requestedYear) = Cint(currYear) Then Response.Write("selected=""selected""")%>><%=currYear%></option>
<%
  currYear = currYear - 1
  Loop%>
