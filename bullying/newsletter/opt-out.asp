...
If Request.Form("Submit") = "Submit" And StrComp(Request.Form("remove"), "remove", vbTextCompare) = 0 And ValidateEmailAddress(Request.Form("emailaddress"))Then
	...
	content = "<p>Unsubscribe Request - Check if in PIX and Unsubscribe from the Bullying Prevention E-News Subscription</p>" & VbCrLf
	content = content & "<p>Email: " & Request.Form("emailaddress") & "</p>"
	content = content & "<p style=""font-size:.8em;""><em>Sent From: http://www.pacer.org/bullying/newsletter/opt-out.asp</em></p>" & VbCrLf
	...
	myMail.Subject = "Check if in PIX and Unsubscribe from the Bullying Prevention E-News Subscription"
...
<p><strong><%=strMail%>
</strong> has been removed from the Bullying Prevention Newsletter Mail List.</p>
<%
showForm = false
